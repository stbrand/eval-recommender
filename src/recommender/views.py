...
@login_required
def reclist_algo_length(request, *args, **kwargs):
	my_set = set()
	...
	return render(request, "recommender/reclist_algo_length.html", context)
...
def surprise_predict(request, *args, **kwargs):
	...
	if request.method == "POST":
		...
		a_form = AlgorithmFilterForm()
		f_form = FilePathForm()
		return redirect("recommender:reclist_algo_length")
...
################## UPLOAD MovieLens Latest Small, Latest Full, 20M, 25M BEGIN###############################
...
def movie_upload(request, *args, **kwargs):
	...
	# context variable that can have different values depending on their context
	context = {
		'title': 'MOVIE UPLOAD',
...
################## UPLOAD MovieLens Latest Small, Latest Full, 20M, 25M END###############################



################## UPLOAD MovieLens 1M, 10M BEGIN###############################
...
################## UPLOAD MovieLens 1M, 10M END###############################




################## UPLOAD MovieLens 100K BEGIN###############################
...
# value = dataset, arg = algo, number = top n
